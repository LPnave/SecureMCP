#!/usr/bin/env python3
"""
Enhanced Secure Prompt MCP Server with NLP and FastMCP Token Authentication
A Model Context Protocol server that uses spaCy for advanced prompt security validation
with JWT token-based authentication using FastMCP's TokenVerifier.
"""
...
from auth_config import get_auth_verifier, check_scope_permission, get_client_info, log_authentication_config

def _get_token_claims() -> Dict:
    """
    Helper function to get token claims from FastMCP access token
    
    Returns:
        Dictionary containing token claims or empty dict if no token
    """
...
# Initialize authentication and MCP server
logger.info("Initializing authentication configuration...")
log_authentication_config()

# Get the appropriate authentication verifier
auth_verifier = get_auth_verifier()

# Initialize MCP server with token authentication
mcp = FastMCP(
    name="Secure Prompt Validator with Token Authentication",
    auth=auth_verifier
)
...
# Tools with token-based authentication and scope validation
@mcp.tool()
async def validate_and_secure_prompt(prompt: str, ctx: Context, context: Optional[str] = None) -> Dict:
    """
    Enhanced prompt validation using NLP analysis.
    Requires scope: prompt:validate
    
    Args:
        prompt: The prompt to validate and secure
        context: Optional JSON string containing additional context
        ctx: FastMCP context for logging (includes token claims)
    
    Returns:
        Dictionary containing validation results and secured prompt
    """
    try:
        # Get token claims using helper function
        token_claims = _get_token_claims()
        client_info = get_client_info(token_claims)
        
        if ctx:
            await ctx.info(f"Starting prompt validation for client: {client_info['client_id']}")
        ...
        return {
            ...
            "nlp_analysis": {},
            "client_info": get_client_info(getattr(ctx, 'token_claims', {}) if ctx else {})
        }

@mcp.tool()
async def analyze_prompt_semantics(prompt: str, ctx: Context) -> Dict:
    """
    Analyze the semantic features of a prompt without security validation.
    Requires scope: prompt:analyze
    ...
    try:
        ...
        if not check_scope_permission(token_claims, "prompt:analyze"):
            return {
                "success": False,
                "error": "Insufficient permissions: prompt:analyze scope required",
                "required_scope": "prompt:analyze",
                "client_scopes": token_claims.get('scopes', [])
            }
        
        client_info = get_client_info(token_claims)
        
        if ctx:
            await ctx.info(f"Starting semantic analysis for client: {client_info['client_id']}")
...
@mcp.tool()
async def update_security_level(level: str, ctx: Context) -> Dict:
    """
    Update the security level for prompt validation.
    Requires scope: security:admin
    ...
    try:
        ...
        if not check_scope_permission(token_claims, "security:admin"):
            return {
                "success": False,
                "error": "Insufficient permissions: security:admin scope required",
        ...
        if ctx:
            await ctx.info(f"Updating security level to: {level} (client: {client_info['client_id']})")
        ...
        return {
            ...
            "updated_by": client_info["client_id"]
        }
...
@mcp.tool()
async def get_security_stats(ctx: Context) -> Dict:
    """
    Get statistics about the enhanced security validator configuration.
    Requires scope: security:stats
    ...
    try:
        ...
        # Check for stats scope
        if not check_scope_permission(token_claims, "security:stats"):
            return {
                "success": False,
                "error": "Insufficient permissions: security:stats scope required",
                "required_scope": "security:stats",
                "client_scopes": token_claims.get('scopes', [])
            }
        ...
        if ctx:
            await ctx.info(f"Retrieving security statistics for client: {client_info['client_id']}")
        ...
        return {
            ...
            "authentication": {
                "mode": auth_verifier.__class__.__name__ if auth_verifier else 'Unknown',
                "client_authenticated": True,
                "client_id": client_info["client_id"]
            },
            "description": "Pure NLP-based Secure Prompt Validator with Token Authentication"
        }
    
    except Exception as e:
        logger.error(f"Error retrieving security stats: {e}")
        if ctx:
            await ctx.error(f"Failed to retrieve security stats: {str(e)}")
        return {
            "success": False,
            "error": str(e)
        }
...
def main():
    """Run the enhanced MCP server with token authentication"""
    logger.info("Starting Enhanced Secure Prompt MCP Server with Token Authentication...")
    logger.info(f"Authentication verifier: {type(auth_verifier).__name__}")
